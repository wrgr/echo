...
const styles = {
  ...
  // Styles for the main header that might be shared or duplicated from App.js structure
  header: {
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
...
const ENCOUNTER_PHASES_CLIENT = {
  0: { name: "Introduction & Initial Presentation", maxScore: 0, coachIntro: (patient) => `Welcome to SUSAN! You're about to meet **${patient.name}**, a **${patient.age}**-year-old. Their main complaint is: "${patient.mainComplaint}". Your goal is to conduct a complete clinical encounter adhering to the Patient-Centered / Biopsychosocial model. Let's begin with **Phase 1: Initiation and Building the Relationship**. What is your first step?"` },
...
function SimulationPage() {
  ...
  const functionUrl = "https://us-central1-clinical-lep-simulator.cloudfunctions.net/susanSimulator"; // Your Firebase Function URL
  ...
  return (
    ...
    <div style={styles.appContainer}>
      {showInstructions && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            ...
            <h2 style={styles.modalTitle}>Welcome to SUSAN: Your Clinical Communication Simulator!</h2>
      ...
      {/* Header (as provided in App.js and copied here) */}
      {/* If this SimulationPage is rendered *within* the App.js's Routes, then the header should be in App.js to avoid duplication */}
      <header style={styles.header}>
        <div style={styles.headerContent}>
          ...
          <div style={styles.titleContainer}>
            <h1 style={styles.title}>SUSAN</h1>
            <h2 style={styles.subtitle}>(Simulating Understanding and Support for Adaptive Narratives)</h2>
          </div>
        </div>
      </header>
      ...
      {patientState && encounterState.currentPhase > 0 && !isFinished && (
        <div style={{width: "100%", backgroundColor: "#e0e0e0", height: "8px"}}>
          <div style={{
            width: `${((encounterState.currentPhase - 1) / (Object.keys(ENCOUNTER_PHASES_CLIENT).length - 2)) * 100}%`, // Minus 2 to exclude intro and complete phases
      ...
      <div style={styles.mainContentWrapper}> {/* Main content area: chat+input and info panel */}
        <div style={styles.chatAndInputContainer}> {/* Left side: Chat window and Input */}
          ...
          <div style={styles.chatWindow} ref={chatWindowRef}>
            ...
            {isLoading && <p style={styles.loadingMessage}>SUSAN is thinking...</p>}
