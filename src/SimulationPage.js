...
function SimulationPage() {
  ...
  // **UPDATED:** Enhanced handlePredefinedPatientChange to support user-generated patients
  const handlePredefinedPatientChange = useCallback((event) => {
    ...
    if (value !== "") {
      ...
      // Check if it's a user-generated patient (starts with "user-")
      ...
      if (patient) {
        ...
        // Mark if this is user-generated for potential special handling
        if (isUserGenerated) {
          console.log("Loaded user-generated patient:", patient.name);
        }
      }
    }
  }, [resetSimulation, userPatients, setPatientState, setSelectedPatientIndex, setMessages, setConversationHistoryForAPI, setEncounterState]);
  ...
  // Effect to refresh user patients when component mounts
  useEffect(() => {
    refreshUserPatients();
  }, [refreshUserPatients]);
  ...
  return (
    ...
    <div style={styles.appContainer}>
          ...
          <div style={styles.modalContent}>
            ...
            <ul style={styles.modalList}>
              <li style={styles.modalListItem}>
                <span style={styles.modalTextBold}>Start a Scenario:</span> Click "Generate New Patient" to get a dynamically created patient profile, or select from the "Load Patient" dropdown. (A default patient is loaded automatically when you start the app.)
              </li>
              <li style={styles.modalListItem}>
      ...
      <div style={styles.controlsContainer}>
        ...
        {/* UPDATED: Enhanced dropdown with user-generated patients */}
        <select
        ...
        >
          ...
          {/* User-generated patients section */}
              ...
              <optgroup label="Your Generated Patients">
                {userPatients.map((patient) => (
                  <option key={`user-${patient.id}`} value={`user-${patient.id}`}>
                    User: {patient.name} ({patient.mainComplaint.substring(0, Math.min(patient.mainComplaint.length, 25))}...)
                  </option>
                ))}
              </optgroup>
            </>
          ...
          {/* Predefined patients section */}
          <optgroup label="Predefined Patients">
        ...
        <button style={styles.controlButton} onClick={() => setShowFullPatientInfo(true)} disabled={isLoading || !patientState}>Full Patient Info</button>
        ...
        <button 
          ...
          disabled={isLoading}
          title="Refresh user-generated patients list"
        >
          Refresh Patients
        </button>
      </div>
