/**
 * ECHO encounter model — client-side source of truth for direct (BYOK) providers.
 *
 * Ported from functions/constants.js so that the Anthropic / Gemini / demo
 * providers can drive a full encounter entirely in the browser, without the
 * Firebase backend. The shape matches the server (phase config + Mini-CEX
 * rubric) so behavior is identical whichever provider the user selects.
 */
...
export const RUBRIC_CATEGORIES = Object.keys(PHASE_RUBRIC);

export const PHASE_COUNT = Object.keys(ENCOUNTER_PHASES).length; // 7 (0..6)
