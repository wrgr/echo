 ...
 * @param {string} postData - The data to post.
 * @param {number} [retries=3] - Number of retry attempts.
 * @return {Promise<string>} A promise that resolves with the response data.
 */
...
const ENCOUNTER_PHASES = {
  ...
  2: {
    ...
    coachIntro: null,
    coachPrompt: `You are in Phase 2 (Information Gathering). Focus on a comprehensive History of Present Illness (HPI), and relevant Past Medical, Social, Family History. Critically, explore the patient's Ideas, Concerns, and Expectations (ICE).`,
 ...
 * @return {Promise<object>} A promise that resolves with the phase score.
 */
async function getPhaseScoreFromGemini(patientState, conversationHistory, phaseName, phaseDescription) {
  ...
  try {
    ...
    // Basic validation for the score structure
...
async function getOverallFeedbackFromGemini(patientState, phaseScores, conversationHistory) {
  ...
  try {
    ...
    const geminiResponse = JSON.parse(rawData);
    const responseText = geminiResponse.candidates[0].content.parts[0].text;
    // No need to remove markdown fences if we expect plain text, but good for safety
 ...
 * @param {"good"|"poor"} responseType - The type of response to generate.
 * @return {Promise<object>} A promise that resolves with {text: string, scoreUpdate: object}.
 */
...
/**
 * Gets free-form advice from Gemini based on provider's query.
...
async function handleGeneratePatient(res) {
  try {
    ...
    // For this example, we'll use a hardcoded placeholder patient.
    const patientData = {
      "name": "Maria Garcia",
      "age": 62,
...
async function handleInteraction(req, res) {
  try {
    ...
    // Calculate current performance ratio (for fidelity)
    const performanceRatio = totalPossibleScore > 0 ? currentCumulativeScore / totalPossibleScore : 1;

    // Use a mutable copy of history for immediate updates within this function's scope
    const updatedConversationHistory = [...conversationHistory]; // Changed to const

    switch (actionType) {
      case "regular_interaction": { // Wrapped in curly braces
        // Check if encounter is already finished
        ...
        // Get Gemini's response (patient or coach) and turn score
        const geminiRegularResponse = await getGeminiResponseForInteraction( // Changed to const
            patientState,
            ...
            currentPhase,
            currentPhaseConfig,
            performanceRatio, // Added trailing comma
        );
        ...
        for (const category in scoreUpdate) {
          if (Object.hasOwnProperty.call(scoreUpdate, category)) { // Added guard-for-in
            currentCumulativeScore += scoreUpdate[category].points;
            totalPossibleScore += PHASE_RUBRIC[category].max;
          }
        }

        // Check for auto-advance if not already marked complete by Gemini
        if (!phaseComplete && currentPhaseConfig.maxTurns > 0 && providerTurnCount >= currentPhaseConfig.maxTurns) {
            ...
            // Add auto-advance message to simulator response if it was a patient response
            if (from === "patient") {
              simulatorResponse += `\n\nCOACH: ${justificationForCompletion}`;
              from = "coach"; // Switch to coach if auto-advancing from patient turn
            } else { // If it was already a coach message from Gemini, just add to it
              simulatorResponse = `COACH: ${justificationForCompletion}\n\n` + simulatorResponse;
            }
        }
      } break; // Added closing brace

      case "get_coach_tip": { // Wrapped in curly braces
        currentPhaseConfig = ENCOUNTER_PHASES[currentPhase];
        // For a general tip, we can just return the current phase's prompt
      ...
      case "inject_provider_response": { // Wrapped in curly braces
        const responseType = latestInput; // This will be "good" or "poor"
        currentPhaseConfig = ENCOUNTER_PHASES[currentPhase];

        // 1. Generate the AI-driven provider response and its *initial* score
        const injectedResponseData = await generateInjectedProviderResponse( // Changed to const
            patientState,
            ...
            responseType, // Added trailing comma
        );
        ...
        // We'll apply this to overall cumulative score.
        // The client should display this score (if desired) and then expect the *next* response as patient/coach.
        ...
        const patientReactionData = await getGeminiResponseForInteraction( // Changed to const
            patientState,
        ...
        // For simplicity, we can apply the score for the patient's reaction to the cumulative.
        // If you need distinct scoring for the 'injected' turn vs 'patient reaction' turn,
        // you'd need to manage `currentCumulativeScore` more granularly or return more data.
        // For now, `scoreUpdate` sent back will be the patient's reaction score.
        for (const category in scoreUpdate) {
          if (Object.hasOwnProperty.call(scoreUpdate, category)) { // Added guard-for-in
            currentCumulativeScore += scoreUpdate[category].points;
            totalPossibleScore += PHASE_RUBRIC[category].max;
        ...
        // Check for auto-advance after patient's reaction
        if (!phaseComplete && currentPhaseConfig.maxTurns > 0 && providerTurnCount >= currentPhaseConfig.maxTurns) {
            phaseComplete = true;
      ...
      case "move_to_next_phase": { // Wrapped in curly braces
        if (currentPhase >= Object.keys(ENCOUNTER_PHASES).length - 1) {
    ...
    if (phaseComplete) {
      ...
      for (const category in fullPhaseScore) {
          if (Object.hasOwnProperty.call(fullPhaseScore, category)) { // Added guard-for-in
      ...
      // If transitioning to the final "Encounter Complete" phase (6)
    ...
    res.status(200).json({
      ...
      justificationForCompletion: justificationForCompletion, // This field is more for internal use or detailed display
